...
func (blockProp *Reactor) ProposeBlock(proposal *types.Proposal, block *types.PartSet, txs []proptypes.TxMetaData) {
	blockProp.Logger.Info("proposing block", "height", proposal.Height, "round", proposal.Round)
...
func (blockProp *Reactor) handleCompactBlock(cb *proptypes.CompactBlock, peer p2p.ID, proposer bool) {
	blockProp.Logger.Info("received proposal", "height", cb.Proposal.Height, "round", cb.Proposal.Round)
	if !proposer {
		// update the peer state to optimistically assume the peer is at this height
		if blockProp.getPeer(peer) != nil {
			// FIXME nil shouldn't happen!
...
func (blockProp *Reactor) broadcastCompactBlock(cb *proptypes.CompactBlock, from p2p.ID) {
	...
	for _, peer := range peers {
		blockProp.Logger.Debug("broadcasting compact block", "peer", peer.peer.ID())
