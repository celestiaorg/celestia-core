...
func TestCheckDeprecated(t *testing.T) {
	...
	// Test that default config generates deprecation warnings for consensus timeout configs and create_empty_blocks
	warnings := cfg.CheckDeprecated()
	expectedWarnings := []string{
		"consensus.timeout_propose is deprecated and will be removed in a future version",
		"consensus.timeout_propose_delta is deprecated and will be removed in a future version",
		"consensus.timeout_prevote is deprecated and will be removed in a future version",
		"consensus.timeout_prevote_delta is deprecated and will be removed in a future version",
		"consensus.timeout_precommit is deprecated and will be removed in a future version",
		"consensus.timeout_precommit_delta is deprecated and will be removed in a future version",
		"consensus.timeout_commit is deprecated and will be removed in a future version",
		"consensus.create_empty_blocks is deprecated and will be removed in a future version",
	}

	// Check mempool configs only if they are set to non-zero values
	if cfg.Mempool.MaxTxBytes != 0 {
		expectedWarnings = append(expectedWarnings, "mempool.max_tx_bytes is deprecated and will be removed in a future version")
	}
	...
	// Test with explicitly set mempool configs that default to zero
	cfg.Mempool.TTLDuration = 10 * time.Second
	cfg.Mempool.TTLNumBlocks = 100
	cfg.Mempool.MaxGossipDelay = 200 * time.Millisecond

	warnings = cfg.CheckDeprecated()
	assert.Contains(t, warnings, "mempool.ttl-duration is deprecated and will be removed in a future version")
	assert.Contains(t, warnings, "mempool.ttl-num-blocks is deprecated and will be removed in a future version")
	assert.Contains(t, warnings, "mempool.max-gossip-delay is deprecated and will be removed in a future version")

	// Test with zero timeout values (should not generate warnings for those specific configs)
	...
	warnings = cfg.CheckDeprecated()
	// CreateEmptyBlocks should still generate a warning since it's always deprecated
	assert.Contains(t, warnings, "consensus.create_empty_blocks is deprecated and will be removed in a future version")

	// The timeout configs should not generate warnings when set to zero
	...
	for _, warning := range timeoutWarnings {
		assert.NotContains(t, warnings, warning)
	}
}
