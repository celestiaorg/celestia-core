...
func TestReactorCreatesBlockWhenEmptyBlocksFalse(t *testing.T) {
	...
	// send a tx
	if err := assertMempool(css[3].txNotifier).CheckTx(&types.CachedTx{Tx: []byte{1, 2, 3}}, nil, mempl.TxInfo{}); err != nil {
		t.Error(err)
	}
...
func waitForAndValidateBlock(
	...
	timeoutWaitGroup(t, n, func(j int) {
		...
		assert.Nil(t, err)
		for _, tx := range txs {
			err := assertMempool(css[j].txNotifier).CheckTx(&types.CachedTx{Tx: tx}, nil, mempl.TxInfo{})
			assert.Nil(t, err)
		}
	}, css)
}
