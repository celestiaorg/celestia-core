...
// initDBs opens or creates the blockstore and state databases and returns the
// raw DB handles. Callers are responsible for wrapping blockStoreDB in a
// store.BlockStore so the wrapper can be configured (e.g. with a background
// compactor) using construction-time options.
//
