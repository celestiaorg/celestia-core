...
func TestOneValidatorChangesSaveLoad(t *testing.T) {
	...
	for i := int64(1); i < highestHeight; i++ {
		...
		header, responses := makeHeaderPartsResponsesValPowerChange(state, power)
		validatorUpdates, err = types.PB2TM.ValidatorUpdates(responses.EndBlock.ValidatorUpdates)
		require.NoError(t, err)
		state, err = sm.UpdateState(state, &header, responses, validatorUpdates)
		require.NoError(t, err)
		err := stateStore.Save(state)
		require.NoError(t, err)
	}
...
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	require.NoError(t, err)
	updatedState, err := sm.UpdateState(state, &block.Header, abciResponses, validatorUpdates)
	...
	assert.NoError(t, err)
	updatedState2, err := sm.UpdateState(updatedState, &block.Header, abciResponses, validatorUpdates)
	assert.NoError(t, err)
	...
	updatedState3, err := sm.UpdateState(updatedState2, &block.Header, abciResponses, validatorUpdates)
	assert.NoError(t, err)
...
func TestProposerPriorityProposerAlternates(t *testing.T) {
	...
	oldState, err = sm.UpdateState(oldState, &block.Header, abciResponses, validatorUpdates)
	assert.NoError(t, err)
	expectedVal1Prio2 = 1
	expectedVal2Prio2 = -1
	expectedVal1Prio = -9
	...
	for i := 0; i < 1000; i++ {
		...
		updatedState, err := sm.UpdateState(oldState, &block.Header, abciResponses, validatorUpdates)
		assert.NoError(t, err)
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		...
		updatedStateInner, err := sm.UpdateState(lastState, &block.Header, abciResponses, validatorUpdates)
		require.NoError(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		block := makeBlock(oldState, oldState.LastBlockHeight+1)
		state, err = sm.UpdateState(state, &block.Header, abciResponses, validatorUpdates)
		require.NoError(t, err)
	}
	...
	for isProposerUnchanged {
		...
		curState, err = sm.UpdateState(curState, &block.Header, abciResponses, validatorUpdates)
		require.NoError(t, err)
	...
	for i := 0; i < 100; i++ {
		...
		updatedState, err = sm.UpdateState(updatedState, &block.Header, abciResponses, validatorUpdates)
		require.NoError(t, err)
...
func TestManyValidatorChangesSaveLoad(t *testing.T) {
	...
	header, responses := makeHeaderPartsResponsesValPubKeyChange(state, pubkey)
...
func TestConsensusParamsChangesSaveLoad(t *testing.T) {
	...
	for i := int64(1); i < highestHeight; i++ {
		...
		header, responses := makeHeaderPartsResponsesParams(state, cp)
		validatorUpdates, err = types.PB2TM.ValidatorUpdates(responses.EndBlock.ValidatorUpdates)
		require.NoError(t, err)
		state, err = sm.UpdateState(state, &header, responses, validatorUpdates)
