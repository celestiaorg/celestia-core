...
type TxPool struct {
	...
	// or if rpcPushMode is true.
	broadcastCh      chan *wrappedTx
	broadcastMtx     sync.Mutex
	txsToBeBroadcast []types.TxKey

	// rpcPushMode, when true, forces RPC-submitted transactions to be queued for
	// broadcast even if config.Broadcast is false. The reactor pushes them as full
	// Txs messages to every connected peer.
	rpcPushMode bool
