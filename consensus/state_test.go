...
func TestStateProposerSelection0(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, rs.ProposalBlock.Hash(), rs.ProposalBlockDAHeader, vss[1:]...)
...
func TestStateProposerSelection2(t *testing.T) {
	...
	for i := int32(0); int(i) < len(vss); i++ {
		...
		// rs := cs1.GetRoundState()
		signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vss[1:]...)
...
func TestStateBadProposal(t *testing.T) {
	...
	propDAH := &propBlock.DataAvailabilityHeader
	...
	proposal := types.NewProposal(vs2.Height, round, -1, propDAH)
	p := proposal.ToProto()
	...
	if err := cs1.SetProposalAndBlock(proposal, propBlock, "some peer"); err != nil {
		t.Fatal(err)
	}
	...
	// wait for proposal
	ensureProposal(proposalCh, height, round, propDAH)
	...
	signAddVotes(cs1, tmproto.PrevoteType, propBlock.Hash(), &propBlock.DataAvailabilityHeader, vs2)
	ensurePrevote(voteCh, height, round)

	// wait for precommit
	ensurePrecommit(voteCh, height, round)
	validatePrecommit(t, cs1, round, -1, vss[0], nil, nil)
	signAddVotes(cs1, tmproto.PrecommitType, propBlock.Hash(), &propBlock.DataAvailabilityHeader, vs2)
}
...
func TestStateOversizedBlock(t *testing.T) {
	...
	proposal := types.NewProposal(height, round, -1, &propBlock.DataAvailabilityHeader)
	p := proposal.ToProto()
	if err := vs2.SignProposal(config.ChainID(), p); err != nil {
		t.Fatal("failed to sign bad proposal", err)
	}
	...
	t.Log("Blocks Sizes", "Limit", cs1.state.ConsensusParams.Block.MaxBytes, "Current", totalBytes)
...
func TestStateFullRound2(t *testing.T) {
	...
	propBlockDAHeader := rs.ProposalBlockDAHeader

	// prevote arrives from vs2:
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash, propBlockDAHeader, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash, propBlockDAHeader, vs2)
	ensurePrecommit(voteCh, height, round)
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	dah := rs.ProposalBlockDAHeader

	ensurePrevote(voteCh, height, round) // prevote

	// we should now be stuck in limbo forever, waiting for more prevotes
	// prevote arrives from vs2:
	signAddVotes(cs1, tmproto.PrevoteType, theBlockHash, dah, vs2)
	ensurePrevote(voteCh, height, round) // prevote
	...
	hash[0] = (hash[0] + 1) % 255
	signAddVotes(cs1, tmproto.PrecommitType, hash, dah, vs2)
	ensurePrecommit(voteCh, height, round) // precommit
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, hash, &rs.LockedBlock.DataAvailabilityHeader, vs2)
	ensurePrevote(voteCh, height, round)
	...
	signAddVotes(cs1, tmproto.PrecommitType, hash, &rs.LockedBlock.DataAvailabilityHeader, vs2)
	ensurePrecommit(voteCh, height, round)
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, hash, &rs.ProposalBlock.DataAvailabilityHeader, vs2)
	ensurePrevote(voteCh, height, round)
	...
	signAddVotes(
		...
		hash,
		&rs.ProposalBlock.DataAvailabilityHeader,
	...
	*/
	...
	if err := cs1.SetProposalAndBlock(prop, propBlock, ""); err != nil {
		t.Fatal(err)
	}
	...
	signAddVotes(
		...
		tmproto.PrecommitType,
		propBlock.Hash(),
		&propBlock.DataAvailabilityHeader,
		vs2) // NOTE: conflicting precommits at same height
	ensurePrecommit(voteCh, height, round)
}
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, theBlockHash, dah, vs2, vs3, vs4)
	...
	signAddVotes(cs1, tmproto.PrecommitType, nil, dah, vs2, vs3, vs4)
	...
	if err := cs1.SetProposalAndBlock(prop, propBlock, "some peer"); err != nil {
		t.Fatal(err)
	}
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash, propDAH, vs2, vs3, vs4)
	...
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash, propDAH, vs2, vs3)
...
func TestStateLockPOLUnlock(t *testing.T) {
	...
	*/
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, nil, dah, vs2, vs4)
	signAddVotes(cs1, tmproto.PrecommitType, theBlockHash, dah, vs3)
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, nil, nil, vs2, vs3, vs4)
	...
	signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vs2, vs3)
...
func TestStateLockPOLUnlockOnUnknownBlock(t *testing.T) {
	...
	*/
	...
	firstDAH := rs.ProposalBlockDAHeader

	ensurePrevote(voteCh, height, round) // prevote

	signAddVotes(cs1, tmproto.PrevoteType, firstBlockHash, firstDAH, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vs2, vs3, vs4)
	...
	secondDAH := &propBlock.DataAvailabilityHeader
	...
	*/
	...
	// now lets add prevotes from everyone else for the new block
	signAddVotes(cs1, tmproto.PrevoteType, secondBlockHash, secondDAH, vs2, vs3, vs4)
	...
	thirdDAH := &propBlock.DataAvailabilityHeader
	...
	*/
	...
	signAddVotes(cs1, tmproto.PrevoteType, thirdPropBlockHash, thirdDAH, vs2, vs3, vs4)
...
func TestStateLockPOLSafety1(t *testing.T) {
	...
	propDAH := rs.ProposalBlockDAHeader

	ensurePrevote(voteCh, height, round)
	validatePrevote(t, cs1, round, vss[0], propBlock.Hash())

	// the others sign a polka but we don't see it
	prevotes := signVotes(tmproto.PrevoteType, propBlock.Hash(), propDAH, vs2, vs3, vs4)
...
func TestStateLockPOLSafety2(t *testing.T) {
	...
	propDAH0 := &propBlock0.DataAvailabilityHeader

	// the others sign a polka but we don't see it
	prevotes := signVotes(tmproto.PrevoteType, propBlockHash0, propDAH0, vs2, vs3, vs4)

	// the block for round 1
	prop1, propBlock1 := decideProposal(cs1, vs2, vs2.Height, vs2.Round+1)
	propBlockHash1 := propBlock1.Hash()
	propDAH1 := &propBlock1.DataAvailabilityHeader
	...
	if err := cs1.SetProposalAndBlock(prop1, propBlock1, "some peer"); err != nil {
		t.Fatal(err)
	}
	ensureNewProposal(proposalCh, height, round)
	...
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash1, propDAH1, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vs2, vs4)
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash1, propDAH1, vs3)
	...
	newProp := types.NewProposal(height, round, 0, &propBlock0.DataAvailabilityHeader)
	p := newProp.ToProto()
	...
	if err := cs1.SetProposalAndBlock(newProp, propBlock0, "some peer"); err != nil {
		t.Fatal(err)
	}
...
func TestSetValidBlockOnDelayedPrevote(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash, propDAH, vs2)

	// vs3 send prevote nil
	signAddVotes(cs1, tmproto.PrevoteType, nil, propDAH, vs3)
	...
	assert.True(t, rs.ValidBlockDAHeader.IsZero())
	assert.True(t, rs.ValidRound == -1)

	// vs2 send (delayed) prevote for propBlock
	signAddVotes(cs1, tmproto.PrevoteType, propBlockHash, propDAH, vs4)
...
// What we want:
// P0 miss to lock B as Proposal Blocks is missing, but set valid block to B after
// receiving delayed Blocks Proposal.
...
func TestEmitNewValidBlockEventOnCommitWithoutBlock(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash, propDAH, vs2, vs3, vs4)
	ensureNewValidBlock(validBlockCh, height, round)

	rs := cs1.GetRoundState()
	assert.True(t, rs.Step == cstypes.RoundStepCommit)
	assert.True(t, rs.ProposalBlock == nil)
	assert.True(t, rs.ProposalBlockDAHeader.Equal(propDAH))
}
...
func TestCommitFromPreviousRound(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrecommitType, propBlockHash, &propBlock.DataAvailabilityHeader, vs2, vs3, vs4)
...
func TestStartNextHeightCorrectlyAfterTimeout(t *testing.T) {
	...
	// add precommits
	signAddVotes(cs1, tmproto.PrecommitType, nil, dah, vs2)
	signAddVotes(cs1, tmproto.PrecommitType, theBlockHash, dah, vs3)
	...
	signAddVotes(cs1, tmproto.PrecommitType, theBlockHash, dah, vs4)
...
func TestResetTimeoutPrecommitUponNewHeight(t *testing.T) {
	...
	// add precommits
	signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vs2)
	signAddVotes(cs1, tmproto.PrecommitType, theBlockHash, dah, vs3)
	signAddVotes(cs1, tmproto.PrecommitType, theBlockHash, dah, vs4)
...
func TestStateHalt1(t *testing.T) {
	...
	signAddVotes(cs1, tmproto.PrevoteType, propBlock.Hash(), propDAH, vs2, vs3, vs4)
	...
	// add precommits from the rest
	signAddVotes(cs1, tmproto.PrecommitType, nil, nil, vs2) // didnt receive proposal
	signAddVotes(cs1, tmproto.PrecommitType, propBlock.Hash(), propDAH, vs3)
	// we receive this later, but vs3 might receive it earlier and with ours will go to commit!
	precommit4 := signVote(vs4, tmproto.PrecommitType, propBlock.Hash(), propDAH)
...
func TestStateOutputsBlocksStats(t *testing.T) {
	// create dummy peer
	cs, _ := randState(1)
	peer := p2pmock.NewPeer(nil)

	block := types.MakeBlock(int64(3), []types.Tx{types.Tx("Hello World")}, nil, nil, types.Messages{}, nil)
	prop := types.NewProposal(1, 0, -1, &block.DataAvailabilityHeader)
	p := prop.ToProto()
	cs.privValidator.SignProposal(cs.state.ChainID, p);
	...
	propMsg := &ProposalMessage{
		Proposal: prop,
	}

	// 1) Send Proposal first to allow further block
	cs.handleMsg(msgInfo{propMsg, peer.ID()})

	// 2) Send Block Message
	...
	case <-cs.statsMsgQueue:
		t.Errorf("should not output stats message after receiving the known block")
...
func TestStateOutputVoteStats(t *testing.T) {
	...
	vote := signVote(vss[1], tmproto.PrecommitType, randBytes, nil)
