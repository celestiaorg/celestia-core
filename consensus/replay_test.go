...
func setupChainWithChangingValidators(t *testing.T, name string, nBlocks int) (*cfg.Config, []*types.Block, []*types.ExtendedCommit, sm.State) {
	...
	height, round := css[0].rs.Height.Load(), css[0].rs.Round.Load()
	...
	signAddVotes(css[0], cmtproto.PrecommitType, rs.GetProposalBlock().Hash(), rs.GetProposalBlockParts().Header(), true, vss[1:nVals]...)
	...
	propBlock, propBlockParts, err := css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	...
	for i := 0; i < nVals+1; i++ {
		...
		signAddVotes(css[0], cmtproto.PrecommitType, rs.GetProposalBlock().Hash(), rs.GetProposalBlockParts().Header(), true, newVss[i])
	}
...
func makeBlocks(n int, state sm.State, privVals []types.PrivValidator) ([]*types.Block, error) {
	...
	for i := 0; i < n; i++ {
		...
		block, _, err := state.MakeBlock(height, types.MakeData(test.MakeNTxs(height, 10)), lastCommit, nil, state.LastValidators.Proposer.Address)
		if err != nil {
			return nil, err
		}
		blocks[i] = block
