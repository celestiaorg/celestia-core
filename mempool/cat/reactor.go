...
func (memR *Reactor) RemovePeer(peer p2p.Peer, reason interface{}) {
	...
	// Clear all memory of seen txs by that peer in one pass; this also
	// resets request bookkeeping for entries whose outstanding WantTx was
	// directed at the peer.
	memR.mempool.seenTracker.RemovePeer(peerID)
...
func (memR *Reactor) Receive(e p2p.Envelope) {
	...
	case *protomem.Txs:
		...
		for _, tx := range protoTxs {
			...
			trackedEntry := memR.mempool.seenTracker.Get(key)
			if trackedEntry != nil && len(trackedEntry.signer) > 0 && trackedEntry.sequence > 0 {
				expectedSeq, haveExpected := memR.querySequenceFromApplication(trackedEntry.signer)
				if haveExpected && trackedEntry.sequence > expectedSeq {
					if trackedEntry.sequence > expectedSeq+maxReceivedBufferSize {
						continue
					}
					// Future sequence within lookahead - buffer it for later
					if memR.receivedBuffer.add(trackedEntry.signer, trackedEntry.sequence, cachedTx, key, txInfo, string(e.Src.ID())) {
						memR.mempool.seenTracker.ClearSequence(key)
					}
					continue
				}
			}
	...
	case *protomem.SeenTx:
		...
		peerID := memR.ids.GetIDForPeer(e.Src.ID())
		// Record peer-has-tx; sequence indexing happens later only when we
		// actually need to hold the entry for gap-fill. This keeps the
		// per-signer queue scoped to txs we cannot yet request.
		...
		case msg.Sequence > expectedSeq:
			// Hold the tx in the per-signer queue until the gap closes.
			// The tracker's per-peer cap bounds memory under a malicious
			// flood of future-sequence SeenTx.
			memR.mempool.PeerHasTxWithSeq(peerID, txKey, msg.Signer, msg.Sequence)
			return
		default:
			memR.Logger.Debug(
...
func (memR *Reactor) broadcastSeenTxWithHeight(txKey types.TxKey, height int64, signer []byte, sequence uint64) {
	...
	for _, peerInfo := range orderedPeers {
		...
		if memR.mempool.seenTracker.Has(txKey, id) {
			continue
		}
...
func (memR *Reactor) processReceivedTx(cachedTx *types.CachedTx, key types.TxKey, txInfo mempool.TxInfo, src p2p.Peer) {
	...
	if err == nil || errors.Is(err, ErrTxInMempool) {
		// Drop the entry from the per-signer queue but preserve the
		// peer-has-tx record so the SeenTx broadcast that follows skips
		// peers already known to have the tx (including the sender).
		memR.mempool.seenTracker.ClearSequence(key)
	}
	if err != nil {
		return
...
func (memR *Reactor) processReceivedBuffer(signer []byte) {
	...
	for {
		...
		if err == nil || errors.Is(err, ErrTxInMempool) {
			memR.mempool.seenTracker.ClearSequence(buffered.txKey)
		}
		if err != nil {
			break
		}
...
func (memR *Reactor) processPendingSeenForSigner(signer []byte) {
	...
	entries := memR.mempool.seenTracker.PendingForSigner(signer)
	if len(entries) == 0 {
		return
	}
	expectedSeq, haveExpected := memR.querySequenceFromApplication(signer)
	...
	for _, entry := range entries {
		...
		if entry.sequence < expectedSeq {
			memR.mempool.seenTracker.ClearSequence(entry.txKey)
			continue
		}
...
func (memR *Reactor) tryRequestQueuedTx(entry *seenEntry) bool {
...
func (memR *Reactor) refreshPendingSeenQueues() {
	// Age out stale entries (peer disconnected, gap never filled) before
	// scheduling more work.
	memR.mempool.seenTracker.Prune(time.Now().UTC().Add(-seenEntryTTL))

	// Collect signers from both the tracker's pending queue and the
	// received buffer.
	seenSigners := make(map[string][]byte)

	for _, signer := range memR.mempool.seenTracker.SignersWithPending() {
...
func (memR *Reactor) querySequenceFromApplication(signer []byte) (uint64, bool) {
	...
	// This prevents transactions from being stuck in the gap-fill queue when the app returns 0.
	if resp.Sequence == 0 {
		return 0, false
	}
...
func (memR *Reactor) findNewPeerToRequestTx(txKey types.TxKey) {
	...
	seenMap := memR.mempool.seenTracker.Peers(txKey)
