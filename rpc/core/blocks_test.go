...
func TestDataCommitmentResults(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		actualCommitment, err := env.DataCommitment(&rpctypes.Context{}, uint64(tc.beginQuery), uint64(tc.endQuery))
		if tc.expectPass {
...
func TestDataRootInclusionProofResults(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		proof, err := env.DataRootInclusionProof(
			&rpctypes.Context{},
			int64(tc.height),
...
func (indexer mockBlockIndexer) Index(types.EventDataNewBlockEvents) error { return nil }
func (indexer mockBlockIndexer) SetLogger(l log.Logger)                    {}
...
func (mockBlockStore) LoadBlockExtendedCommit(height int64) *types.ExtendedCommit { return nil }

func (mockBlockStore) LoadSeenCommit(height int64) *types.Commit { return nil }
func (mockBlockStore) PruneBlocks(height int64, state sm.State) (uint64, int64, error) {
	return 0, 0, nil
}

func (mockBlockStore) SaveBlockWithExtendedCommit(block *types.Block, blockParts *types.PartSet, seenCommit *types.ExtendedCommit) {
}
...
func (store mockBlockStore) Close() error {
	return nil
}

func (store mockBlockStore) DeleteLatestBlock() error {
	return nil
}
