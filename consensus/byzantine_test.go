...
func TestByzantinePrevoteEquivocation(t *testing.T) {
	...
	bcs.doPrevote = func(height int64, round int32) {
		...
		if height == prevoteHeight {
			...
			prevote1, err := bcs.signVote(cmtproto.PrevoteType, bcs.rs.GetProposalBlock().Hash(), bcs.rs.GetProposalBlockParts().Header(), nil)
			require.NoError(t, err)
	...
	lazyProposer.decideProposal = func(height int64, round int32) {
		...
		switch {
		case lazyProposer.rs.Height.Load() == lazyProposer.state.InitialHeight:
			// We're creating a proposal for the first block.
			// The commit is empty, but not nil.
			extCommit = &types.ExtendedCommit{}
		case lazyProposer.rs.GetLastCommit().HasTwoThirdsMajority():
			// Make the commit from LastCommit
			veHeightParam := types.ABCIParams{VoteExtensionsEnableHeight: height}
			extCommit = lazyProposer.rs.GetLastCommit().MakeExtendedCommit(veHeightParam)
		...
		block, _, err := lazyProposer.blockExec.CreateProposalBlock(
			ctx, lazyProposer.rs.Height.Load(), lazyProposer.state, extCommit, proposerAddr)
		require.NoError(t, err)
		...
		proposal := types.NewProposal(height, round, lazyProposer.rs.ValidRound.Load(), propBlockID)
		p := proposal.ToProto()
		if err := lazyProposer.privValidator.SignProposal(lazyProposer.state.ChainID, p); err == nil {
			...
			for i := 0; i < int(blockParts.Total()); i++ {
				...
				lazyProposer.sendInternalMessage(msgInfo{&BlockPartMessage{lazyProposer.rs.Height.Load(), lazyProposer.rs.Round.Load(), part}, ""})
			}
...
func byzantineDecideProposalFunc(ctx context.Context, t *testing.T, height int64, round int32, cs *State, sw *p2p.Switch) {
	...
	require.NoError(t, err)
	polRound, propBlockID = cs.rs.ValidRound.Load(), types.BlockID{Hash: block2.Hash(), PartSetHeader: blockParts2.Header()}
