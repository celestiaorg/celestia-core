...
func TestStateProposerSelection0(t *testing.T) {
	cs1, vss := randState(4, mockipfs.MockedIpfsAPI())
...
func TestStateProposerSelection2(t *testing.T) {
	cs1, vss := randState(4, mockipfs.MockedIpfsAPI()) // test needs more work for more than 3 validators
	height := cs1.Height
	newRoundCh := subscribe(cs1.eventBus, types.EventQueryNewRound)
...
func TestStateEnterProposeNoPrivValidator(t *testing.T) {
	cs, _ := randState(1, mockipfs.MockedIpfsAPI())
...
func TestStateBadProposal(t *testing.T) {
	cs1, vss := randState(2, mockipfs.MockedIpfsAPI())
	height, round := cs1.Height, cs1.Round
	vs2 := vss[1]
...
func TestStateFullRound1(t *testing.T) {
	cs, vss := randState(1, mockipfs.MockedIpfsAPI())
	height, round := cs.Height, cs.Round
...
func TestStateLockNoPOL(t *testing.T) {
	...
	*/
	...
	cs2, _ := randState(2, mockipfs.MockedIpfsAPI()) // needed so generated block is different than locked block
...
func TestStateLockPOLRelock(t *testing.T) {
	...
	*/
	...
	cs2 := newState(cs1.state, vs2, counter.NewApplication(true), mockipfs.MockedIpfsAPI())
	prop, propBlock := decideProposal(cs2, vs2, vs2.Height, vs2.Round+1)
	if prop == nil || propBlock == nil {
		t.Fatal("Failed to create proposal block with vs2")
	}
...
func TestStateLockPOLUnlockOnUnknownBlock(t *testing.T) {
	...
	*/
	...
	// before we timeout to the new round set the new proposal
	cs3 := newState(cs1.state, vs3, counter.NewApplication(true), mockipfs.MockedIpfsAPI())
...
func TestStateOutputVoteStats(t *testing.T) {
	cs, vss := randState(2, mockipfs.MockedIpfsAPI())
	// create dummy peer
	peer := p2pmock.NewPeer(nil)
