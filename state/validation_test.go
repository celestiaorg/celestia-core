...
func TestValidateBlockHeader(t *testing.T) {
	...
	testCases := []struct {
		...
		{"first LastCommit contains signatures", func(block *types.Block) {
			block.LastCommit = types.NewCommit(0, 0, types.BlockID{}, []types.CommitSig{types.NewCommitSigAbsent()}, nil)
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		*/
		var err error
		state, _, lastCommit, _, err = makeAndCommitGoodBlock(state, height, lastCommit, proposerAddr, blockExec, privVals, nil)
...
func TestValidateBlockCommit(t *testing.T) {
	...
	lastCommit := types.NewCommit(0, 0, types.BlockID{}, nil, nil)
	wrongSigsCommit := types.NewCommit(1, 0, types.BlockID{}, nil, nil)
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		*/
		...
		state, blockID, lastCommit, headerHash, err = makeAndCommitGoodBlock(
			state,
			height,
			lastCommit,
			proposerAddr,
		...
		*/
		...
		wrongSigsCommit = types.NewCommit(goodVote.Height, goodVote.Round,
			blockID, []types.CommitSig{goodVote.CommitSig(), badVote.CommitSig()}, headerHash)
	}
}
...
func TestValidateBlockEvidence(t *testing.T) {
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		*/
		...
		state, _, lastCommit, _, err = makeAndCommitGoodBlock(
			state,
			height,
			lastCommit,
			proposerAddr,
