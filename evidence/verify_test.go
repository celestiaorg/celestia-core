...
func TestVerifyLightClientAttack_Lunatic(t *testing.T) {
	...
	commit, err := types.MakeCommit(conflictingHeader.Hash(), 10, 1, voteSet, conflictingPrivVals, defaultEvidenceTime)
	require.NoError(t, err)
	...
	trustedCommit, err := types.MakeCommit(trustedHeader.Hash(), 10, 1, trustedVoteSet, privVals, defaultEvidenceTime)
	require.NoError(t, err)
...
func TestVerifyLightClientAttack_Equivocation(t *testing.T) {
	...
	voteSet := types.NewVoteSet(evidenceChainID, 10, 1, tmproto.SignedMsgType(2), conflictingVals)
	commit, err := types.MakeCommit(conflictingHeader.Hash(), 10, 1, voteSet, conflictingPrivVals[:4], defaultEvidenceTime)
	require.NoError(t, err)
	ev := &types.LightClientAttackEvidence{
		ConflictingBlock: &types.LightBlock{
			SignedHeader: &types.SignedHeader{
	...
	trustedCommit, err := types.MakeCommit(trustedHeader.Hash(), 10, 1, trustedVoteSet, conflictingPrivVals, defaultEvidenceTime)
	require.NoError(t, err)
	trustedSignedHeader := &types.SignedHeader{
		Header: trustedHeader,
		Commit: trustedCommit,
...
func TestVerifyLightClientAttack_Amnesia(t *testing.T) {
	...
	commit, err := types.MakeCommit(conflictingHeader.Hash(), 10, 0, voteSet, conflictingPrivVals, defaultEvidenceTime)
	require.NoError(t, err)
	ev := &types.LightClientAttackEvidence{
		ConflictingBlock: &types.LightBlock{
			SignedHeader: &types.SignedHeader{
...
func TestVerifyDuplicateVoteEvidence(t *testing.T) {
	...
	blockID2 := []byte("blockhash2")
	blockID3 := []byte("blockhash")
	blockID4 := []byte("blockhash")
...
func makeVote(
	...
	round int32, step int, headerHash []byte, time time.Time) *types.Vote {
...
func makeHeaderRandom(height int64) *types.Header {
	return &types.Header{
		...
		LastHeaderHash:     []byte("headerhash"),
