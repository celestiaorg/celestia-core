...
func (r *orderedReactor) Receive(e p2p.Envelope) {
	...
	r.received = append(r.received, string(envMsg.Txs[0])) //nolint:staticcheck // SA1019: test payload deliberately uses the deprecated repeated field
}
...
func (r *panicReactor) Receive(e p2p.Envelope) {
	...
	envMsg := e.Message.(*mempool.Txs)
	msgStr := string(envMsg.Txs[0]) //nolint:staticcheck // SA1019: test payload deliberately uses the deprecated repeated field
...
func (r *precheckReactor) Receive(e p2p.Envelope) {
	...
	r.received = append(r.received, string(m.Txs[0])) //nolint:staticcheck // SA1019: test payload deliberately uses the deprecated repeated field
}
