...
func TestSignerRawBytesRejectsOtherChainID(t *testing.T) {
	for _, tc := range getSignerTestCases(t) {
		tc := tc
		t.Cleanup(func() {
			if err := tc.signerServer.Stop(); err != nil {
		...
		// Mimic a multi-chain remote signer (e.g. Horcrux) that signs for
		// whatever chain ID the request names.
		var called atomic.Bool
		tc.signerServer.SetRequestHandler(func(
			privVal types.PrivValidator,
			req privvalproto.Message,
			_ string,
		) (privvalproto.Message, error) {
			if r := req.GetSignRawBytesRequest(); r != nil {
				called.Store(true)
				return DefaultValidationRequestHandler(privVal, req, r.ChainId)
			}
			return DefaultValidationRequestHandler(privVal, req, tc.chainID)
		})

		sig, err := tc.signerClient.SignRawBytes("other-chain", cmtrand.Str(12), cmtrand.Bytes(32))
		require.ErrorContains(t, err, "chain ID mismatch")
		require.Nil(t, sig)
		require.False(t, called.Load(), "request for another chain must not reach the remote signer")
	}
}
