...
func TestValidatorSet_VerifyCommit_All(t *testing.T) {
	...
	commit := NewCommit(vote.Height, vote.Round, []CommitSig{vote.CommitSig()}, vote.HeaderHash)
	...
	testCases := []struct {
		...
		{"good", chainID, vote.HeaderHash, vote.Height, commit, false},

		{"wrong signature (#0)", "EpsilonEridani", vote.HeaderHash, vote.Height, commit, true},
		{"wrong block ID", chainID, tmrand.Bytes(tmhash.Size), vote.Height, commit, true},
		{"wrong height", chainID, vote.HeaderHash, vote.Height - 1, commit, true},

		{"wrong set size: 1 vs 0", chainID, vote.HeaderHash, vote.Height,
			NewCommit(vote.Height, vote.Round, []CommitSig{}, vote.HeaderHash), true},

		{"wrong set size: 1 vs 2", chainID, vote.HeaderHash, vote.Height,
			NewCommit(vote.Height, vote.Round,
				[]CommitSig{vote.CommitSig(), {CommitFlag: CommitFlagAbsent}}, vote.HeaderHash), true},

		{"insufficient voting power: got 0, needed more than 666", chainID, vote.HeaderHash, vote.Height,
			NewCommit(vote.Height, vote.Round, []CommitSig{{CommitFlag: CommitFlagAbsent}}, vote.HeaderHash), true,
		},

		{"wrong signature (#0)", chainID, vote.HeaderHash, vote.Height,
			NewCommit(vote.Height, vote.Round, []CommitSig{vote2.CommitSig()}, vote.HeaderHash), true},
	}

	for _, tc := range testCases {
		tc := tc
		t.Run(tc.description, func(t *testing.T) {
			err := vset.VerifyCommit(tc.chainID, tc.headerHash, tc.height, tc.commit)
			if tc.expErr {
			...
			err = vset.VerifyCommitLight(tc.chainID, tc.headerHash, tc.height, tc.commit)
			if tc.expErr {
...
func TestValidatorSet_VerifyCommit_CheckAllSignatures(t *testing.T) {
	...
	commit, err := MakeCommit(headerHash, h, 0, voteSet, vals, time.Now())
	require.NoError(t, err)
	...
	err = valSet.VerifyCommit(chainID, headerHash, h, commit)
	if assert.Error(t, err) {
...
func TestValidatorSet_VerifyCommitLight_ReturnsAsSoonAsMajorityOfVotingPowerSigned(t *testing.T) {
	...
	err = valSet.VerifyCommitLight(chainID, headerHash, h, commit)
	assert.NoError(t, err)
}
...
func TestValidatorSet_VerifyCommitLightTrusting(t *testing.T) {
	var (
		...
		commit, err                   = MakeCommit(headerHash, 1, 1, voteSet, vals, time.Now())
