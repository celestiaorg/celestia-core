...
// CanonicalizeVote transforms the given Proposal to a CanonicalProposal.
func CanonicalizeCompactProposal(chainID string, proposal *tmproto.Proposal) tmproto.CanonicalCompactProposal {
	return tmproto.CanonicalCompactProposal{
		Type:              tmproto.ProposalType,
		Height:            proposal.Height,       // encoded as sfixed64
		Round:             int64(proposal.Round), // encoded as sfixed64
		POLRound:          int64(proposal.PolRound),
		...
		Timestamp:         proposal.Timestamp,
		ChainID:           chainID,
		CompactBlockParts: CanonicalizePartSetHeader(proposal.CompactBlockParts),
	}
}
