...
func TestBeginBlockValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		// block for height 2
		block, _ := state.MakeBlock(2, makeTxs(2), []byte("proof"), lastCommit, nil, state.Validators.GetProposer().Address)
...
func TestBeginBlockByzantineValidators(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		block, _ := state.MakeBlock(10, makeTxs(2), []byte("proof"), lastCommit, nil, state.Validators.GetProposer().Address)
		block.Time = now
